...
export default function SignupPage() {
  ...
  const submit = async (e: any) => {
    ...
    try {
      ...
      const { accessToken, token, userId, email, name } = res.data;
      const jwt = accessToken || token;
      setAuth(jwt, { id: userId, email: email || form.email, name: name || form.name });
    ...
    <div className="min-h-screen flex items-center justify-center p-4">
      ...
      >
        ...
        <div className="glass p-8 rounded-2xl">
          ...
          <p className="text-gray-400 text-sm mb-7">Get started with PayFlow today</p>

          <form onSubmit={submit} className="space-y-4">
            <div>
              <label className="label">Full Name</label>
              <input id="input-name" className="input" type="text" placeholder="Jane Doe"
            ...
            <div>
              ...
              <div className="relative">
                <input id="input-password" className="input pr-10" type={showPw ? 'text' : 'password'} placeholder="Min 8 characters"
            ...
            <button id="btn-signup" type="submit" disabled={busy} className="btn-primary w-full mt-2">
        ...
        <p className="text-center text-xs text-gray-600 mt-4">
          A welcome notification will be sent via <span className="text-gray-500">notification-service</span>
        </p>
      </motion.div>
    </div>
  );
